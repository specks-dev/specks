...
- **Prefix conventions (use these consistently)**:
  - **`step-N`**: execution step anchors, e.g. `{#step-0}`, `{#step-1}`, `{#step-2}`
...
#### 4) `**Depends on:**` lines for execution step dependencies (beads-compatible)
...
- Omit the line entirely for steps with no dependencies (typically Step 0)
...
- Dependencies must reference valid step anchors within the document (validated by `specks validate`)

**Beads mapping:** Each `**Depends on:**` reference becomes a blocking dependency edge when syncing to beads via `specks beads sync`.

---

#### 5) Beads linkage: root and step beads

Each speck has a **root bead** (epic/molecule) and each execution step may have a **step bead** (child of the root). These are written by `specks beads sync`; do not invent bead IDs manually.

**Root bead (Plan Metadata):**
- Stored in the Plan Metadata table as the **Beads Root** row, value = `` `bd-xxxx` `` (or project-specific prefix, e.g. `` `gt-abc1` ``).
- Created by sync as the parent of all step beads; enables `bd ready --parent <root-id>` workflows.

**Step beads:**
- Each step block may have a `**Bead:**` line recording that step’s bead ID (a child of the root).
- **Format:** `` **Bead:** `bd-xxxx` `` or with hierarchical suffix for substep beads, e.g. `` **Bead:** `bd-xxxx.1` ``. Bead IDs are variable-length (e.g. `bd-a3f8e9`, `bd-xyz.2`). Prefix may not be `bd-` if the project uses a different Beads prefix.
- **Placement:** after `**Depends on:**` (if present), before `**Commit:**`. Added/updated automatically by sync.
- **Substeps:** By default substeps do not get their own beads; they are reflected in the parent step’s bead description. With `specks beads sync --substeps children`, substeps get child beads and their IDs are written to each substep’s `**Bead:**` line (IDs assigned by Beads, not assumed).

**Bead ID format (validation):** IDs match pattern `^[a-z0-9][a-z0-9-]*-[a-z0-9]+(\.[0-9]+)*$` (prefix-hash with optional `.N` suffix for hierarchy). When Beads is available, validation may also verify the bead exists via `bd show`.

---

#### 5b) Optional `**Beads:**` hints block (per step)

You may add an optional, machine-readable block inside a step to guide bead creation when syncing:

```markdown
**Beads:** type=task, priority=1, labels=backend, estimate_minutes=60
```

- **type**: `task` | `feature` | `bug` | `epic` | `chore` (and project custom types)
- **priority**: 1–4 (P1–P4)
- **labels**: comma-separated (no spaces after commas)
- **estimate_minutes**: integer

These hints are used by `specks beads sync` to set bead type, priority, labels, and time estimate. Omit the block to use defaults. Do **not** put bead IDs here; sync writes IDs to the `**Bead:**` line.

---

#### 6) `**References:**` lines are required for every execution step
...
<!-- Step 0 typically has no dependencies (it is the root) -->
<!-- **Bead:** `bd-xxxx` is added by `specks beads sync` after linking -->
...
<!-- **Bead:** `bd-xxxx.1` - substeps use dotted notation -->
...
<!-- **Bead:** `bd-xxxx.2` - substeps use dotted notation -->
