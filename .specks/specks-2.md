...
##### Step 8.3.6.5: Streaming Agent Output with Fixed Spinner {#step-8-3-6-5}

**Depends on:** #step-8-3-6

**Commit:** `feat(cli): add streaming agent output with fixed spinner like Claude Code`

**References:** [D15] CLI interaction, (#d15-cli-interaction)

**Problem Statement:**

The current CLI feedback is terrible:
- User sees a static spinner for 30+ seconds
- Then ALL content dumps at once
- No sense of progress, no visibility into what the agent is doing
- Nothing like the Claude Code experience where you see content streaming in real-time

**Design Decision [D25]: Streaming Agent Output with Fixed Spinner** {#d25-streaming-spinner}

Agent output should stream to the terminal as it's generated, with a fixed spinner/elapsed timer at the bottom. This matches the Claude Code experience where:
1. Content streams in real-time at the cursor position
2. A spinner with elapsed time stays fixed below the streaming content
3. The spinner updates (pulses/animates) while content streams above it

**Required UX:**

```
Clarifying requirements for: create a python calculator...

I'll analyze this idea and explore the codebase for context.

Looking at the project structure, I see this is an empty project
with specks configuration...

For a Python CLI calculator, there are several design decisions:
1. Input method: command-line arguments vs interactive REPL
2. Expression parsing: simple operations vs complex expressions
3. Error handling: division by zero, invalid syntax
...

⠸ Clarifier analyzing... [12.3s]
```

The spinner line stays fixed at the bottom while content streams above it. When the agent completes, the spinner transforms to a success/failure message.

**Dependencies to add:**
```toml
crossterm = "0.28"  # Terminal control for cursor positioning
```

**Artifacts:**
- `crates/specks/src/agent.rs` - Add streaming invocation method
- `crates/specks/src/streaming.rs` - Streaming output with fixed spinner
- Updated `crates/specks/Cargo.toml` - Add crossterm dependency
- Updated planning loop to use streaming invocation

**Tasks:**

*New streaming module:*
- [x] Add `crossterm = "0.28"` to `crates/specks/Cargo.toml`
- [x] Create `crates/specks/src/streaming.rs` with:
  - `StreamingDisplay` struct that manages terminal output
  - `start(&mut self, message: &str)` - Begin streaming with spinner
  - `write_line(&mut self, line: &str)` - Write content line (above spinner)
  - `update_spinner(&mut self)` - Update spinner animation and elapsed time
  - `finish_success(&mut self)` - Transform spinner to success message
  - `finish_error(&mut self, msg: &str)` - Transform spinner to error message
  - Use crossterm for cursor positioning:
    - Save cursor position before spinner
    - Clear spinner line when new content arrives
    - Reprint spinner after content
    - Handle terminal resize gracefully

*Streaming agent invocation:*
- [x] Add `invoke_agent_streaming()` method to `AgentRunner`:
  ```rust
  ...
  - Use `BufReader` to read stdout line-by-line
  - For each line: call `display.write_line(line)` and `display.update_spinner()`
  - On completion: return full output for parsing

*Planning loop integration:*
- [x] Update `run_clarifier()` in planning loop to use streaming invocation
- [x] Update `run_planner()` to use streaming invocation
- [x] Update `run_critic()` to use streaming invocation
- [x] Pass `StreamingDisplay` through the loop phases

*Spinner behavior:*
- [x] Spinner animates: `⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏` (braille pattern)
- [x] Show elapsed time: `[12.3s]`
- [x] Update every 100ms minimum
- [x] On success: `✓ Clarifier complete [30.8s]` (green)
- [x] On failure: `✗ Clarifier failed [5.2s]` (red)

*Terminal control:*
- [x] Handle non-TTY gracefully (just print lines without spinner)
- [x] Handle Ctrl+C: clean up terminal state before exit
- [x] Use alternate screen? (decision: probably not, keep simple scrollback)

**Tests:**
- [x] Unit test: `StreamingDisplay` writes content correctly
- [x] Unit test: Spinner updates elapsed time
- [x] Unit test: Non-TTY mode falls back to simple output
- [x] Integration test: Streaming invocation captures all output

**Manual Test Script:**
```bash
# Before: static spinner, content dumps at end
specks plan "add a greeting command"
# After: content streams in real-time, spinner stays at bottom

# Verify:
# 1. See content appear line-by-line as agent generates it
# 2. Spinner stays fixed at bottom with elapsed time
# 3. On completion, spinner transforms to success message
# 4. Ctrl+C cleanly exits with terminal state restored
```

**Checkpoint:**
- [x] `cargo build` succeeds
- [x] `cargo nextest run` passes
- [x] `crossterm` dependency added to Cargo.toml
- [ ] Manual test: Agent content streams in real-time
- [ ] Manual test: Spinner stays fixed at bottom
- [ ] Manual test: Elapsed time updates continuously
- [ ] Manual test: Ctrl+C exits cleanly

**Rollback:**
- Remove streaming.rs, revert agent.rs and planning loop changes
