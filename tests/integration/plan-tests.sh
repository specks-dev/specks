#!/usr/bin/env bash
# Integration tests for the `specks plan` command.
# Exercises the planning loop with the mock claude CLI.
#
# Run from repo root: tests/integration/plan-tests.sh
# Or with verbose output: VERBOSE=1 tests/integration/plan-tests.sh

set -e
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
SPECKS_BIN="${REPO_ROOT}/target/debug/specks"
CLAUDE_MOCK="${REPO_ROOT}/tests/bin/claude-mock-plan"

# Test temp directory
TEST_TMPDIR="${TEST_TMPDIR:-$(mktemp -d "${TMPDIR:-/tmp}/specks-plan-tests.XXXXXX")}"
...
# Build specks if needed
build_specks() {
    if [[ ! -f "$SPECKS_BIN" ]]; then
        echo "Building specks..."
        (cd "$REPO_ROOT" && cargo build --quiet)
    fi
}

# Create a test project directory with .specks initialized
create_test_project() {
    local project_dir="$1"
    mkdir -p "$project_dir/.specks"
...
EOF
    ...
    cat > "$project_dir/.specks/config.toml" << 'EOF'
[specks]
...
run_test() {
    ...
    # Reset mock state
    rm -f "$MOCK_STATE"
    export SPECKS_CLAUDE_MOCK_PLAN_STATE="$MOCK_STATE"
...
test_plan_requires_init() {
    ...
    # Running plan without .specks should fail with exit code 9
    if "$SPECKS_BIN" plan "test idea" 2>&1; then
...
test_plan_help() {
    "$SPECKS_BIN" plan --help 2>&1 | grep -q "iterative agent collaboration"
}
...
test_plan_json_error() {
    ...
    local output
    output=$("$SPECKS_BIN" --json plan "test idea" 2>&1 || true)
...
test_plan_claude_not_installed() {
    ...
    local exit_code=0
    "$SPECKS_BIN" plan "test idea" 2>&1 || exit_code=$?
...
test_plan_with_mock_completes() {
    ...
    verbose "Running: specks plan 'add a test feature' --name test-feature"
    local output exit_code=0
    output=$("$SPECKS_BIN" plan "add a test feature" --name test-feature 2>&1) || exit_code=$?
    verbose "Output: $output"
    ...
    # Check that speck file was created
    if [[ ! -f "${test_dir}/.specks/specks-test-feature.md" ]]; then
        verbose "Speck file not created"
        return 1
    fi

    verbose "Speck file created successfully"
    return 0
}
...
test_plan_with_abort() {
    ...
    export SPECKS_CLAUDE_MOCK_PLAN_ABORT="true"

    local exit_code=0
    "$SPECKS_BIN" plan "test idea" --name abort-test 2>&1 || exit_code=$?

    unset SPECKS_CLAUDE_MOCK_PLAN_ABORT
...
test_plan_json_output() {
    ...
    local output
    output=$("$SPECKS_BIN" --json plan "test idea" --name json-test 2>&1)
    verbose "Output: $output"
    ...
    if echo "$output" | jq -e '.status == "ok"' >/dev/null 2>&1; then
        echo "$output" | jq -e '.data.speck_name' >/dev/null
