...
# Run from repo root: SPECKS_BD_STATE=<tmp> tests/integration/bd-fake-tests.sh
...
export SPECKS_BD_STATE="${SPECKS_BD_STATE:-$(mktemp -d "${TMPDIR:-/tmp}/specks-bd-fake-state.XXXXXX")}"
...
reset_state() {
  rm -f "$SPECKS_BD_STATE/issues.json" "$SPECKS_BD_STATE/deps.json"
  bd init >/dev/null
}
...
test_show_array_compatible() {
  # Specks must accept both array and single object; simulate array response
  reset_state
  local id
...
test_pull_data_status_field() {
  ...
  local out
  out=$(bd show "$id" | normalize_show)
  # Consumer (specks beads pull) maps status "closed" -> checkbox checked
...
main() {
  need_jq
  echo "Using SPECKS_BD_STATE=$SPECKS_BD_STATE"
