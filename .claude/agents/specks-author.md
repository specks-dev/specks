...
3. **Plan Creation**: Produce detailed, actionable implementation plans following the structure defined in @.specks/specks-skeleton.md.
...
- Structure the plan according to @.specks/specks-skeleton.md
...
- Always write plans to files in the @.specks directory
- Use descriptive filenames that reflect the feature or task (e.g., `specks-typescript-support.md`, `specks-error-handling-refactor.md`)
...
- If the skeleton template at @.specks/specks-skeleton.md is not found, ask the user to provide it or create a sensible default structure
...
## Beads-ready planning (when execution will use Beads)

Plans can be synced to Beads via `specks beads sync` so that each step becomes a bead (child of a single root epic). To make plans sync cleanly:

- **Stable anchors**: Give every execution step an explicit anchor (e.g. `{#step-0}`, `{#step-1}`). Anchors are the stable identity for steps; titles may change.
- **Depends on**: Every step except the first should have a `**Depends on:**` line that references step anchors (e.g. `**Depends on:** #step-0`). This becomes the dependency graph in Beads.
- **Optional Beads hints**: When priority or type is obvious, add a `**Beads:**` block to the step, e.g. `**Beads:** type=task, priority=1, labels=backend`. Do not put bead IDs in the plan; `specks beads sync` creates beads and writes IDs back to the `**Bead:**` line and Plan Metadata (Beads Root).
- **Never invent bead IDs**: The plan should not contain `**Bead:**` or Beads Root IDs until after the user runs `specks beads sync`. You may leave the Beads Root row in Plan Metadata with a placeholder (e.g. "written by sync"); sync will fill it.
...
- [ ] Dependencies between steps are clearly stated (e.g. `**Depends on:**` with step anchors)
- [ ] Testing and verification criteria are included
- [ ] The plan has been written to the appropriate file in @.specks
- [ ] Execution steps have stable anchors and no manually invented bead IDs
