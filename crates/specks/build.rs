//! Build script for specks CLI
//!
//! Captures build-time information:
//! - Git commit hash (SPECKS_COMMIT)
//! - Build date (SPECKS_BUILD_DATE)
//! - Rust compiler version (SPECKS_RUSTC_VERSION)
//!
...
fn main() {
    ...
    println!("cargo::rustc-env=SPECKS_COMMIT={}", commit);

    // Capture build date (YYYY-MM-DD format)
    let build_date = get_build_date();
    println!("cargo::rustc-env=SPECKS_BUILD_DATE={}", build_date);

    // Capture rustc version
    let rustc_version = get_rustc_version();
    println!("cargo::rustc-env=SPECKS_RUSTC_VERSION={}", rustc_version);
