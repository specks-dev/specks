...
fn check_and_rotate_log(project_root: &std::path::Path) -> Result<(bool, Option<String>), String> {
    ...
    let log_path = project_root.join(".specks/specks-implementation-log.md");
...
fn perform_log_rotation(
    ...
    let log_path = project_root.join(".specks/specks-implementation-log.md");
    let archive_dir = project_root.join(".specks/archive");
...
mod tests {
    ...
    fn test_beads_close_triggers_rotation_on_oversized_log() {
        ...
        // Set up .specks directory structure
        ...
        let archive_dir = specks_dir.join("archive");
        ...
        assert!(
            new_content.contains("# Specks Implementation Log"),
    ...
    fn test_beads_close_no_log_file() {
        ...
        // Set up .specks directory but no log
        let specks_dir = temp_path.join(".specks");
        fs::create_dir_all(&specks_dir).unwrap();
