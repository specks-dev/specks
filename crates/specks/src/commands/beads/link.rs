//! Implementation of the `specks beads link` command (Spec S07)
...
pub fn run_link(
    ...
    let step_info = find_step_by_anchor(&speck, &step_anchor);
...
fn find_step_by_anchor(speck: &specks_core::Speck, anchor: &str) -> Option<(usize, bool)> {
    // Check main steps
