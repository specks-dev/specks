//! Implementation of the `specks beads status` command (Spec S08)
...
pub fn run_beads_status(
    ...
    let files = match &file {
        ...
        None => find_specks(&project_root).unwrap_or_default(),
    };
    ...
    for path in files {
        ...
        let file_status = get_file_beads_status(&path, &speck, &beads);
...
fn get_file_beads_status(path: &Path, speck: &Speck, beads: &BeadsCli) -> FileBeadsStatus {
    let name = speck_name_from_path(path).unwrap_or_else(|| "unknown".to_string());
    ...
    FileBeadsStatus {
        file,
        name,
        root_bead_id: speck.metadata.beads_root_id.clone(),
        steps_complete,
        steps_total,
        steps: steps_status,
    }
