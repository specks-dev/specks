...
impl Config {
    ...
    pub fn load_from_project(project_root: &Path) -> Result<Self, SpecksError> {
        let config_path = project_root.join(".specks").join("config.toml");
...
/// Find the project root by searching upward for `.specks/` directory
///
...
pub fn find_project_root_from(start: PathBuf) -> Result<PathBuf, SpecksError> {
    let mut current = start;
    loop {
        let specks_dir = current.join(".specks");
        if specks_dir.is_dir() {
            return Ok(current);
        }
        match current.parent() {
            Some(parent) => current = parent.to_path_buf(),
            None => return Err(SpecksError::NotInitialized),
        }
    }
}

/// Find all speck files in the .specks directory
///
/// Per [D03], speck files match `specks-*.md` except reserved files.
pub fn find_specks(project_root: &Path) -> Result<Vec<PathBuf>, SpecksError> {
    let specks_dir = project_root.join(".specks");
    if !specks_dir.is_dir() {
        return Err(SpecksError::NotInitialized);
    }
    ...
    for entry in entries {
        ...
        if let Some(filename) = path.file_name().and_then(|n| n.to_str()) {
            // Check if it matches specks-*.md pattern and is not reserved
...
/// Extract speck name from filename (remove prefix and extension)
///
/// e.g., "specks-1.md" -> "1", "specks-feature-x.md" -> "feature-x"
pub fn speck_name_from_path(path: &Path) -> Option<String> {
    path.file_name()
        .and_then(|n| n.to_str())
        .and_then(|filename| {
            if filename.starts_with("specks-") && filename.ends_with(".md") {
                let name = &filename[7..filename.len() - 3]; // Remove "specks-" and ".md"
                if !name.is_empty() {
...
mod tests {
    ...
    fn test_is_reserved_file() {
        assert!(is_reserved_file("specks-skeleton.md"));
        assert!(is_reserved_file("specks-implementation-log.md"));
        assert!(!is_reserved_file("specks-1.md"));
        assert!(!is_reserved_file("specks-feature.md"));
    }

    #[test]
    fn test_speck_name_from_path() {
        assert_eq!(
            speck_name_from_path(Path::new("specks-1.md")),
            Some("1".to_string())
        );
        assert_eq!(
            speck_name_from_path(Path::new("specks-feature-x.md")),
            Some("feature-x".to_string())
        );
        assert_eq!(
            speck_name_from_path(Path::new(".specks/specks-refactor.md")),
            Some("refactor".to_string())
        );
        assert_eq!(speck_name_from_path(Path::new("other.md")), None);
        assert_eq!(speck_name_from_path(Path::new("specks-.md")), None);
    }

    #[test]
    fn test_default_config() {
        let config = Config::default();
        assert_eq!(config.specks.validation_level, "normal");
        assert!(!config.specks.show_info);
        assert_eq!(config.specks.naming.prefix, "specks-");
        assert!(config.specks.beads.enabled);
    }
}
