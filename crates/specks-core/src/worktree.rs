...
/// Extract speck slug from branch name.
///
/// Branch format: `specks/<slug>-<timestamp>` where timestamp is exactly 15 chars (YYYYMMDD-HHMMSS).
/// This function strips the `specks/` prefix and the last 16 chars (hyphen + timestamp).
///
/// Examples:
/// - "specks/auth-20260208-143022" -> "auth"
/// - "specks/auth-v2-20260208-143022" -> "auth-v2"
/// - "specks/1-20260208-143022" -> "1"
pub fn slug_from_branch(branch: &str) -> String {
    let without_prefix = branch.strip_prefix("specks/").unwrap_or(branch);
...
fn find_existing_worktree(
    ...
    let expected_slug = derive_speck_slug(&config.speck_path);

    // Filter to matching speck_slug and sort by timestamp (most recent first)
    let mut matching: Vec<DiscoveredWorktree> = all_worktrees
        .into_iter()
        .filter(|wt| wt.speck_slug == expected_slug)
        .collect();
...
/// (worktree_path, branch_name, speck_slug). Worktree metadata is returned
...
pub fn list_worktrees(repo_root: &Path) -> Result<Vec<DiscoveredWorktree>, SpecksError> {
    ...
    for line in stdout.lines() {
        if let Some(path_str) = line.strip_prefix("worktree ") {
            ...
            // Only include branches with "specks/" prefix
            if branch_ref.starts_with("specks/") {
                if let Some(wt_path) = current_path.take() {
                    ...
                    let is_main = canonical_root
                        ...
                        .unwrap_or(false);
                    if !is_main {
                        let speck_slug = slug_from_branch(branch_ref);
...
///
/// The `speck_slug` is derived from the branch name by stripping the `specks/` prefix
...
pub struct DiscoveredWorktree {
    ...
    pub branch: String,
    /// Speck slug derived from branch name
...
mod tests {
    ...
    fn test_slug_from_branch() {
        ...
        // Multi-hyphen slug
        assert_eq!(
            slug_from_branch("specks/auth-v2-20260208-143022"),
            "auth-v2"
        );
        ...
        // Long slug
        assert_eq!(
            slug_from_branch("specks/feature-name-with-many-parts-20260208-143022"),
    ...
    fn test_list_worktrees_returns_discovered_worktrees() {
        ...
        Command::new("git")
            .current_dir(temp_dir)
            .args(["branch", "specks/test-20260208-120000"])
            .output()
            .expect("Failed to create branch");

        let worktree_path = worktrees_dir.join("specks__test-20260208-120000");
        Command::new("git")
            .current_dir(temp_dir)
            .args([
                "worktree",
        ...
        assert_eq!(worktrees[0].branch, "specks/test-20260208-120000");
        assert_eq!(worktrees[0].speck_slug, "test");
        assert_eq!(worktrees[0].base_branch, "main");
    }

    #[test]
    fn test_find_worktree_by_speck_finds_matching_worktree() {
        use std::process::Command;
        use tempfile::TempDir;
        ...
        let worktree_path = worktrees_dir.join("specks__auth-20260208-120000");
        Command::new("git")
            .current_dir(temp_dir)
            .args([
                "worktree",
    ...
    #[test]
    fn test_find_worktree_by_speck_returns_none_when_not_found() {
        use tempfile::TempDir;
